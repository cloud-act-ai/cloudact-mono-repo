...
# test_guru_234.sh - Test bootstrap, onboarding, and cost pipeline for guru_234
# Usage: ./test_guru_234.sh [local|stage|prod]
...
if [ -z "$ENV" ]; then
    echo "Usage: ./test_guru_234.sh [local|stage|prod]"
    exit 1
fi

# Configuration
TENANT_ID="guru_234"
DESCRIPTION="Test Tenant guru_234 for $ENV verification"
DATE=$(date +%Y-%m-%d)

echo -e "${BLUE}----------------------------------------------------------------${NC}"
echo -e "${BLUE}Running Test for guru_234 on $ENV${NC}"
echo -e "${BLUE}----------------------------------------------------------------${NC}"
...
# 2. Onboard Tenant
echo "Step 2: Onboarding Tenant guru_234..."
...
if [[ "$response" == *"pipeline_logging_id"* ]] || [[ "$response" == *"SUCCESS"* ]]; then
    echo -e "${GREEN}Test Complete! Pipeline triggered successfully for guru_234.${NC}"
else
