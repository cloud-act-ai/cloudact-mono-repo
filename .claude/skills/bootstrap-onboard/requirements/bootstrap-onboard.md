...
System bootstrap and organization onboarding for CloudAct. Covers two-phase onboarding (Frontend Supabase/Stripe + Backend BigQuery), the central `organizations` dataset (21 meta tables), per-org datasets (30+ tables), API key management, org sync, and fiscal year configuration.
...
### Central "organizations" Dataset (21 Meta Tables)
...
|-----------|------|---------|
| Bootstrap status | curl | `curl /api/v1/admin/bootstrap/status` — verify all 21 meta tables exist |
...
| `/bigquery-ops` | BigQuery schema management. Bootstrap creates 21 meta tables. |
