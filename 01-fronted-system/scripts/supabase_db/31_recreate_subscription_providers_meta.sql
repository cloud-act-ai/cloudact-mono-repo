-- =============================================
-- Migration: Recreate SaaS Subscription Provider Meta Table
-- Purpose: Fix missing subscription_providers_meta table
-- Run: Execute in Supabase SQL Editor or via migrate.sh
-- =============================================
...
CREATE TABLE subscription_providers_meta (
    ...
    -- Custom provider fields (for user-added providers)
    display_name VARCHAR(100),  -- Custom display name
    category VARCHAR(50),       -- Provider category
    is_custom BOOLEAN DEFAULT FALSE,  -- Whether this is a custom provider
...
CREATE INDEX idx_subscription_providers_meta_org_id
    ON subscription_providers_meta(org_id);

CREATE INDEX idx_subscription_providers_meta_enabled
    ON subscription_providers_meta(org_id, is_enabled);

CREATE INDEX idx_subscription_providers_meta_provider
    ON subscription_providers_meta(provider_name);
...
COMMENT ON TABLE subscription_providers_meta IS 'Tracks which subscription providers are enabled per org. Full plan details stored in BigQuery.';
COMMENT ON COLUMN subscription_providers_meta.provider_name IS 'Provider identifier: canva, chatgpt_plus, slack, figma, etc.';
COMMENT ON COLUMN subscription_providers_meta.is_enabled IS 'Whether this provider is enabled for the org';
COMMENT ON COLUMN subscription_providers_meta.enabled_at IS 'When the provider was first enabled';
COMMENT ON COLUMN subscription_providers_meta.is_custom IS 'Whether this is a user-added custom provider';
