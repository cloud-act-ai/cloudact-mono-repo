...
export function SetupPrompt() {
  return (
    <div className="rounded-xl border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800/30 p-6">
      <h3 className="mb-4 text-lg font-medium text-gray-900 dark:text-white">Getting Started</h3>
      <div className="space-y-4">
        {STEPS.map((step) => (
          <div key={step.title} className="flex gap-4">
            <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700">
              <step.icon className="h-5 w-5 text-gray-400 dark:text-slate-400" />
            </div>
            <div>
