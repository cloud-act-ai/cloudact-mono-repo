...
export function AdvancedSettings({
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-sm font-medium text-slate-300">Advanced Settings</h3>
      ...
      {/* Max Tokens */}
      <div>
        <label className="mb-2 block text-sm text-slate-400">Max Output Tokens</label>
        <select
          value={maxTokens}
      ...
      {/* Max History */}
      <div>
        <label className="mb-2 block text-sm text-slate-400">Max History Messages</label>
        <select
      ...
      <div className="space-y-3">
        <label className="flex items-center justify-between">
          <span className="text-sm text-slate-400">Include org context in system prompt</span>
          <input
            type="checkbox"
            checked={includeOrgContext}
            onChange={(e) => onChange("includeOrgContext", e.target.checked)}
            className="rounded border-slate-600 accent-[#90FCA6]"
          />
        </label>
        <label className="flex items-center justify-between">
          <span className="text-sm text-slate-400">Enable conversation memory</span>
          <input
            type="checkbox"
      ...
      <div>
        ...
        />
        <p className="mt-1 text-xs text-slate-600">{systemPromptExtra.length}/2000</p>
      </div>
    </div>
  )
}
