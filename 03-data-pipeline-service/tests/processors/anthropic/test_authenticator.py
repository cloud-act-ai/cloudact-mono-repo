"""
Tests for Anthropic Authenticator
...
class TestAnthropicAuthenticator:
    """Test cases for AnthropicAuthenticator."""

    @pytest.mark.asyncio
    async def test_authenticate_success(self, mock_settings):
        """Test successful API key decryption."""
        with patch("src.core.processors.anthropic.authenticator.BigQueryClient") as MockBQClient, \
             patch("src.core.processors.anthropic.authenticator.decrypt_value") as mock_decrypt:
            ...
            auth = AnthropicAuthenticator("test_org_123")
    ...
    async def test_authenticate_no_credentials(self, mock_settings):
        ...
        with patch("src.core.processors.anthropic.authenticator.BigQueryClient") as MockBQClient:
            ...
            from src.core.processors.anthropic.authenticator import AnthropicAuthenticator
            auth = AnthropicAuthenticator("nonexistent_org")

            with pytest.raises(ValueError, match="No valid Anthropic credentials found"):
...
class TestAnthropicValidationProcessor:
    """Test cases for Anthropic validation processor."""

    @pytest.mark.asyncio
    async def test_execute_success(self, mock_settings):
        """Test successful validation execution."""
        with patch("src.core.processors.anthropic.validation.AnthropicAuthenticator") as MockAuth:
            mock_auth = MagicMock()
