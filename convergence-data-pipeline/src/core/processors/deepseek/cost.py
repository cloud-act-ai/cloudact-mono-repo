"""
DeepSeek Cost Processor

Transforms raw usage data to cost using model pricing from ref_model_pricing table.
Reads from BigQuery (deepseek_usage_daily_raw), writes to BigQuery (deepseek_cost_daily).
...
class CostProcessor:
    """
    Transforms DeepSeek usage data to cost records.

    Reads from: {org_slug}_{env}.deepseek_usage_raw
    Writes to:  {org_slug}_{env}.deepseek_cost
    """
    ...
    async def execute(
        ...
        Args:
            step_config: Step configuration containing:
                - config.source_table: Source table name (default: deepseek_usage_raw)
                - config.destination_table: Destination table name (default: deepseek_cost)
        ...
        source_table = config.get("source_table", "deepseek_usage_daily_raw")
        destination_table = config.get("destination_table", "deepseek_cost_daily")
        ...
        self.logger.info(
            f"Transforming DeepSeek usage to cost for {org_slug}",
            extra={
                "org_slug": org_slug,
        ...
        try:
            ...
            self.logger.error(f"DeepSeek cost transform error: {e}", exc_info=True)
            return {
                "status": "FAILED",
                "provider": "DEEPSEEK",
                "error": str(e)
    ...
    async def _load_pricing_from_bq(
        ...
        try:
            ...
            if pricing:
                self.logger.info(f"Loaded pricing for {len(pricing)} DeepSeek models from ref_model_pricing")
                return pricing
            else:
