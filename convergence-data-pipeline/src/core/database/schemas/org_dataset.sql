-- ============================================================================
-- ORGANIZATION DATASET - Per-Organization Operational Data Schema
-- ============================================================================
-- Project: gac-prod-471220
-- Dataset: {org_slug} (dynamic per org, e.g., customer_abc123)
-- Purpose: Org-isolated operational data for pipeline execution, logs, and data quality
-- Version: 3.0.0
-- Created: 2025-11-17
-- Updated: 2025-11-24 - Renamed from organization to org
--
-- Isolation: One dataset per organization for complete data isolation
-- Security: IAM-based access control per org dataset
-- ============================================================================

-- ============================================================================
-- CREATE DATASET (Dynamic per organization)
-- ============================================================================
-- Example: CREATE SCHEMA IF NOT EXISTS `gac-prod-471220.customer_abc123`
-- This is created programmatically during organization onboarding
-- ============================================================================

-- ============================================================================
-- TABLE 1: org_meta_pipeline_runs (DEPRECATED - MOVED TO CENTRAL ORGANIZATIONS DATASET)
-- ============================================================================
-- Purpose: Pipeline execution metadata and logging
-- Scope: NOW CENTRALIZED in organizations.org_meta_pipeline_runs for all orgs
-- NOTE: This table definition is kept for reference only. In production,
--       org_meta_pipeline_runs is created in the central 'organizations' dataset,
--       not in per-org datasets. All pipeline runs across all orgs
--       are logged to organizations.org_meta_pipeline_runs with org_slug column.
-- ============================================================================
CREATE TABLE IF NOT EXISTS `{project_id}.{org_slug}.org_meta_pipeline_runs` (
  -- Primary Identifiers
...
OPTIONS(
  description="Pipeline execution metadata and logging for org operations",
  labels=[("category", "pipeline_execution"), ("tier", "operational")]
);

-- ============================================================================
-- TABLE 2: org_meta_step_logs
-- ============================================================================
-- Purpose: Detailed step-by-step execution logs for pipeline debugging
-- Scope: Per-org operational logs
-- ============================================================================
CREATE TABLE IF NOT EXISTS `{project_id}.{org_slug}.org_meta_step_logs` (
  -- Primary Identifiers
  log_id STRING NOT NULL,                             -- Unique log entry ID (UUID)
  pipeline_logging_id STRING NOT NULL,                -- Foreign key to org_meta_pipeline_runs
...
-- ============================================================================
-- TABLE 3: org_meta_dq_results
-- ============================================================================
-- Purpose: Data quality validation results
-- Scope: Per-org data quality metrics
-- ============================================================================
CREATE TABLE IF NOT EXISTS `{project_id}.{org_slug}.org_meta_dq_results` (
  -- Primary Identifiers
...
-- ============================================================================
-- TABLE 4: org_pipeline_configs
-- ============================================================================
-- Purpose: Per-org pipeline scheduling configurations
-- Scope: Org-specific pipeline settings (copied from centralized config)
-- ============================================================================
CREATE TABLE IF NOT EXISTS `{project_id}.{org_slug}.org_pipeline_configs` (
  -- Primary Identifiers
...
OPTIONS(
  description="Per-org pipeline scheduling configurations",
  labels=[("category", "pipeline_config"), ("tier", "operational")]
);

-- ============================================================================
-- TABLE 5: org_scheduled_pipeline_runs
-- ============================================================================
-- Purpose: Track scheduled pipeline execution history
-- Scope: Org-specific execution history
-- ============================================================================
CREATE TABLE IF NOT EXISTS `{project_id}.{org_slug}.org_scheduled_pipeline_runs` (
  -- Primary Identifiers
  ...
  pipeline_logging_id STRING,                         -- References org_meta_pipeline_runs
...
OPTIONS(
  description="Scheduled pipeline execution history per org",
  labels=[("category", "pipeline_execution"), ("tier", "operational")]
);
...
--
-- 1. org_meta_pipeline_runs: Clustered by (status, pipeline_template, started_at)
--    - Fast lookup by status and template
--    - Efficient time-range queries
--
-- 2. org_meta_step_logs: Clustered by (pipeline_logging_id, step_number, status)
--    - Fast step lookup for specific pipeline runs
--    - Efficient debugging queries
--
-- 3. org_meta_dq_results: Clustered by (pipeline_logging_id, check_type, check_passed)
--    - Fast DQ result lookup
--    - Efficient failure queries
--
-- 4. org_pipeline_configs: Clustered by (next_run_time, is_active)
--    - Fast "due to run" queries
--    - Efficient scheduler lookups
--
-- 5. org_scheduled_pipeline_runs: Clustered by (state, scheduled_time)
...
-- NOTE: org_meta_pipeline_runs is now centralized in organizations dataset
CREATE OR REPLACE VIEW `{project_id}.{org_slug}.recent_pipeline_runs` AS
SELECT
  pipeline_logging_id,
  pipeline_name,
  pipeline_template,
...
-- NOTE: org_meta_pipeline_runs is now centralized in organizations dataset
CREATE OR REPLACE VIEW `{project_id}.{org_slug}.failed_pipelines` AS
SELECT
  pipeline_logging_id,
  pipeline_name,
  pipeline_template,
...
CREATE OR REPLACE VIEW `{project_id}.{org_slug}.dq_failures` AS
SELECT
  dq_result_id,
  pipeline_logging_id,
  check_name,
  ...
  failure_rate,
  executed_at
FROM `{project_id}.{org_slug}.org_meta_dq_results`
...
--   rows_processed,
--   log_message
-- FROM `{project_id}.{org_slug}.org_meta_step_logs`
