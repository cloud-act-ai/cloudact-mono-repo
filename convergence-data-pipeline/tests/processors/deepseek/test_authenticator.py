"""
Tests for DeepSeek Authenticator
...
class TestDeepSeekAuthenticator:
    """Test cases for DeepSeekAuthenticator."""

    @pytest.mark.asyncio
    async def test_authenticate_success(self, mock_settings):
        """Test successful API key decryption."""
        with patch("src.core.processors.deepseek.authenticator.BigQueryClient") as MockBQClient, \
             patch("src.core.processors.deepseek.authenticator.decrypt_value") as mock_decrypt:
            ...
            auth = DeepSeekAuthenticator("test_org_123")
    ...
    async def test_authenticate_no_credentials(self, mock_settings):
        ...
        with patch("src.core.processors.deepseek.authenticator.BigQueryClient") as MockBQClient:
            ...
            from src.core.processors.deepseek.authenticator import DeepSeekAuthenticator
            auth = DeepSeekAuthenticator("nonexistent_org")

            with pytest.raises(ValueError, match="No valid DeepSeek credentials found"):
...
class TestDeepSeekValidationProcessor:
    """Test cases for DeepSeek validation processor."""

    @pytest.mark.asyncio
    async def test_execute_success(self, mock_settings):
        """Test successful validation execution."""
        with patch("src.core.processors.deepseek.validation.DeepSeekAuthenticator") as MockAuth:
            mock_auth = MagicMock()
            ...
            from src.core.processors.deepseek.validation import DeepSeekValidationProcessor
