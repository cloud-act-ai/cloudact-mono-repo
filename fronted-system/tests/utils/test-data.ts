...
export function generateTestUser(flowName: string) {
    return {
        ...
        password: (process.env?.TEST_PASSWORD as string) || 'testpass123',
...
export const TEST_CONFIG = {
    baseUrl: (process.env?.TEST_BASE_URL as string) || 'http://localhost:3000',
    timeout: parseInt((process.env?.TEST_TIMEOUT as string) || '60000'),
